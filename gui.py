...
from toe import load_parameters, initial_state, initialize_value_table, is_tie, is_win, select_next_step, update_value,\
    save_parameters
...
class MyApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.episodes = tk.IntVar(value=100)
        ...
        self.wins_label = tk.Label(self, textvariable=self.wins)
        self.losses_label = tk.Label(self, textvariable=self.losses)
        self.draws_label = tk.Label(self, textvariable=self.draws)

        # use a loop to build my field buttons
        for i in range(9):
            button = tk.Button(self, text=str('-'),
        ...
        self.wins_label.grid(row=5, column=2)
        self.losses_label.grid(row=5, column=3)
        self.draws_label.grid(row=5, column=4)
    ...
    def do_button(self, i):
        ...
        if self.player_turn and self.episodes.get() > 0:
            if self.game_state[i] == str(empty_tic):
                ...
                if str(empty_tic) in self.game_state:
                ...
                if is_tie(self.game_state, str(agent_tic), str(opponent_tic), str(empty_tic)):
                    self.desc_label['text'] = 'Tie game'
                    self.draws.set(self.draws.get() + 1)
                    self.episodes.set(self.episodes.get() - 1)
                    self.game_state = initial_state()
                elif is_win(self.game_state, str(opponent_tic)):
                    self.desc_label['text'] = 'You have won'
                    self.losses.set(self.losses.get() + 1)
                    self.episodes.set(self.episodes.get() - 1)
                    self.game_state = initial_state()
                elif is_win(self.game_state, str(agent_tic)):
